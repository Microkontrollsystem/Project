...
void lcd_msg(char *c)
{
	while(*c != 0)      // Vent til alle strenger er sendt til LCD
	lcd_dwr(*c++);		// Send strengen til LCD
}

void lcd_rig_sh()
{
	lcd_cmd(0x1C);      // Kommando for høyre Shift
	delay(400);
}

void lcd_lef_sh()
{
	lcd_cmd(0x18);      // Kommando for venstre skift
	delay(200);
}
